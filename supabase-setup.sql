...
-- Cross-campus discovery is explicit opt-in. Adding this column with a false
-- default keeps every historical campus-only post private during migration.
alter table public.community_posts
...
-- verified ConCourse member. This supports the read-only cross-campus feed
-- without exposing school keys, block rows, or unpublished media metadata.
...
-- Verified members may discover opted-in published posts from other universities, but
-- creation remains school-scoped because every publisher above derives the
-- destination school from private.verified_school_key(). Cross-campus cards
-- expose a pseudonymous username and verified institution name; school-only
-- profile fields stay private. Likes, comments, and bookmarks use the same
-- interaction RPCs as the campus feed.
...
-- ---------------------------------------------------------------------------
-- Verified-campus marketplace with opt-in global discovery
-- ---------------------------------------------------------------------------
...
-- block checks. Cross-university visibility is false by default and exposes a
-- pseudonymous seller identity only. Money is stored as integer minor units
-- with an ISO 4217-style three-letter currency code. A browser can create an
-- awaiting-payment order, but only a trusted service-role payment webhook can record held, released,
...
create table if not exists public.marketplace_listings (
  ...
  global_visible boolean not null default false,
...
-- Cross-university discovery is always explicit. Adding the column with a
-- false default keeps every existing and newly-created listing campus-only
-- until its verified seller opts in.
alter table public.marketplace_listings
  add column if not exists global_visible boolean not null default false;

-- Marketplace introductions can cross school boundaries only while their
-- source listing remains actively shared. Campus conversations keep their
-- original behavior and all existing rows stay in the campus context.
...
-- Cross-university cards intentionally use a smaller public identity. A
-- school-only display name, avatar, major, phone number, or social link must
-- never become visible merely because the seller shared a listing globally.
...
-- Global discovery is a separate RPC so campus defaults never become broader
-- by accident. Only active listings whose seller explicitly opted in appear,
-- and the caller's own university remains in the existing campus feed.
...
begin
  ...
  if not is_transaction_participant then
    ...
    if caller_school is null
       or not (
         (
           listing_row.school_key = caller_school
           and listing_row.status in ('active', 'reserved')
         )
  ...
  result := case
    when is_cross_school_view
      then private.marketplace_global_listing_json(p_listing_id, caller)
    else private.marketplace_listing_json(p_listing_id, caller)
  end;
...
begin
  ...
  -- A revoked seller can always make a listing private. Opting in or remaining
  -- opted in requires the seller's current verified home-school membership.
  if p_visible and (
    caller_school is null
    or caller_school <> listing_row.school_key
  ) then
...
-- A global-market introduction is a narrow exception to campus-only chat. It
-- exists only while the referenced listing is active and explicitly shared,
-- and it continues to honor verification, bilateral blocks, and both users'
-- Allow messages settings on every read and write.
  ...
  select exists (
    ...
    where conversation.id = p_conversation_id
      ...
      and not exists (
        ...
        where (
          block.blocker_id = p_user_id
          and block.blocked_id = other_membership.user_id
        ) or (
          block.blocked_id = p_user_id
          and block.blocker_id = other_membership.user_id
        )
      )
      and (
        (
...
begin
  ...
  select
    ...
    profile.username,
    case
      when mine.context_type = 'campus' and member.profile_visibility = 'school'
        then member.display_name
      else null
    end,
    case
...
begin
  ...
  where conversation.id = p_conversation_id
    ...
    and (
      (
        conversation.context_type = 'campus'
        and conversation.school_key = private.verified_school_key()
      )
      or (
        conversation.context_type = 'marketplace'
        and conversation.marketplace_listing_id is not null
...
-- Cross-campus community posts may carry the seller's opted-in global listing.
-- The marketplace JSON builder keeps the seller pseudonymous and repeats the
-- active-listing, verified-school, and privacy checks.
drop function if exists public.get_cross_school_feed(integer, integer, boolean, uuid);
create or replace function public.get_cross_school_feed(
  p_limit integer default 30,
  p_offset integer default 0,
...
-- Refresh the Supabase API schema after the complete setup finishes.
